...
use engine_traits::{
    self, Error, Mutable, Result, ValueType, WriteBatchExt, WriteBatchVecExt, WriteOptions,
};
pub use rocksdb::WriteBatch as RawWriteBatch;
use rocksdb::{DBValueType, Writable, WriteBatchIter, WriteBatchRef, DB};

pub const WRITE_BATCH_BATCH_MAX_SIZE: usize = 16;
pub const WRITE_BATCH_MAX_DATA_SIZE: usize = 8 * 1024;
pub const WRITE_BATCH_MAX_BATCH_COUNT: usize = 16;
...
impl RocksWriteBatchVec {
    ...
    fn check_switch_batch(&mut self) {
        ...
        if cur_count >= WRITE_BATCH_BATCH_MAX_SIZE {
            self.index += 1;
...
impl engine_traits::WriteBatch for RocksWriteBatchVec {
    ...
    fn should_write_to_engine(&self) -> bool {
        self.index >= WRITE_BATCH_MAX_BATCH_COUNT
    }

    fn append(&mut self, data: &[u8]) -> Result<()> {
        if data.len() > WRITE_BATCH_MAX_DATA_SIZE {
            let wb_ref = WriteBatchRef::new(data);
            if wb_ref.count() > WRITE_BATCH_BATCH_MAX_SIZE * 2 {
...
impl WriteBatchVecExt<RocksEngine> for RocksWriteBatch {
    fn write_batch_vec(e: &RocksEngine, cap: usize) -> RocksWriteBatch {
...
impl WriteBatchVecExt<RocksEngine> for RocksWriteBatchVec {
    fn write_batch_vec(e: &RocksEngine, cap: usize) -> RocksWriteBatchVec {
...
impl<'a, 'b> Iterator for RocksWriteBatchIter<'a, 'b> {
    ...
    fn next(&mut self) -> Option<(ValueType, &'a str, &'b [u8], &'b [u8])> {
        self.inner.next().map(|(t, cf_id, key, value)| {
            ...
            let cf = self.cfs[cf_id as usize].as_str();
...
mod tests {
    ...
    use super::*;
    use engine_traits::WriteBatchVecExt;
    ...
    fn test_should_write_to_engine() {
        ...
        wb.put(b"aaa", b"bbb").unwrap();
        assert!(wb.should_write_to_engine());
        let mut wb: RocksWriteBatchVec = WriteBatchVecExt::write_batch_vec(&engine, 1024);
        for _i in 0..WRITE_BATCH_MAX_BATCH_COUNT * WRITE_BATCH_BATCH_MAX_SIZE {
            wb.put(b"aaa", b"bbb").unwrap();
        }
        assert!(!wb.should_write_to_engine());
        wb.put(b"aaa", b"bbb").unwrap();
